...
<div class="space-y-6">
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div class="bg-white p-6 rounded-lg shadow-lg border-l-4 border-blue-500 border-b-2 border-gray-300">
            ...
            <p class="text-2xl font-bold text-blue-600">R$
                <?php echo number_format($totalPendente ?? 0, 2, ',', '.'); ?></p>
        </div>
        <div class="bg-white p-6 rounded-lg shadow-lg border-l-4 border-green-500 border-b-2 border-gray-300">
            ...
            <p class="text-2xl font-bold text-green-600"><?php echo number_format($mediaConsumo ?? 0, 2, ',', '.'); ?>
                m³</p>
        </div>
    ...
    <form action="index.php?page=agua" method="POST" class="space-y-4 bg-white p-6 rounded-lg shadow-lg">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div>
                <label for="consumo_m3" class="block text-sm font-medium text-gray-700">Consumo (m³)</label>
                <input type="number" name="consumo_m3" id="consumo_m3" step="0.01" required
                    class="mt-1 block w-full rounded-md border-gray-400 border-2 shadow-sm focus:border-[#072a3a] focus:ring focus:ring-[#072a3a] focus:ring-opacity-50">
            </div>
            <div>
            ...
            <div>
                ...
                <input type="number" name="valor_fatura" id="valor_fatura" step="0.01" required
                    class="mt-1 block w-full rounded-md border-gray-400 border-2 shadow-sm focus:border-[#072a3a] focus:ring focus:ring-[#072a3a] focus:ring-opacity-50">
            </div>
            <div>
            ...
            <div>
                <label for="tipo_consumo" class="block text-sm font-medium text-gray-700">Tipo de Consumo</label>
                <input type="text" name="tipo_consumo" id="tipo_consumo"
                    class="mt-1 block w-full rounded-md border-gray-400 border-2 shadow-sm focus:border-[#072a3a] focus:ring focus:ring-[#072a3a] focus:ring-opacity-50">
            </div>
            <div>
                <label for="num_instalacao" class="block text-sm font-medium text-gray-700">Nº da Instalação</label>
                <input type="text" name="num_instalacao" id="num_instalacao"
                    class="mt-1 block w-full rounded-md border-gray-400 border-2 shadow-sm focus:border-[#072a3a] focus:ring focus:ring-[#072a3a] focus:ring-opacity-50">
            </div>
            <div>
        ...
        <div class="flex items-center justify-end space-x-2">
            <button type="submit" name="salvar"
                class="bg-[#072a3a] hover:bg-[#051e2b] text-white font-bold py-2 px-4 rounded-md shadow-sm">
                Salvar
            </button>
        </div>
    </form>
    ...
    <div class="bg-white p-6 rounded-lg shadow-lg">
        ...
        <form action="index.php" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
            ...
            <div>
                ...
                <input type="text" name="filtro_secretaria" id="filtro_secretaria"
                    value="<?= htmlspecialchars($_GET['filtro_secretaria'] ?? '') ?>"
                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
            </div>
            <div>
                <label for="filtro_tipo_consumo" class="block text-sm font-medium text-gray-700">Tipo de Consumo</label>
                <input type="text" name="filtro_tipo_consumo" id="filtro_tipo_consumo"
                    value="<?= htmlspecialchars($_GET['filtro_tipo_consumo'] ?? '') ?>"
                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
            </div>
            <div>
                <label for="filtro_num_instalacao" class="block text-sm font-medium text-gray-700">Nº da
                    Instalação</label>
                <input type="text" name="filtro_num_instalacao" id="filtro_num_instalacao"
                    value="<?= htmlspecialchars($_GET['filtro_num_instalacao'] ?? '') ?>"
                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
            </div>
            <div class="flex items-end">
                <button type="submit"
                    class="bg-gray-200 hover:bg-gray-300 text-gray-800 font-bold py-2 px-4 rounded-md shadow-sm">
                    Filtrar
                </button>
                <a href="index.php?page=agua"
                    class="bg-gray-200 hover:bg-gray-300 text-gray-800 font-bold py-2 px-4 rounded-md shadow-sm ml-2">
                    Limpar
                </a>
            </div>
        </form>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Mês
                        </th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                            Unidade</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                            Consumo (m³)</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Multa
                        </th>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Tipo
                            de Consumo</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Nº da
                            Instalação</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                            Vencimento</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    <?php foreach ($registros as $registro): ?>
                        <tr id="row-<?= htmlspecialchars($registro['id_agua']) ?>">
                            <td class="px-6 py-4 whitespace-nowrap"><?php echo htmlspecialchars($registro['mes'] ?? '-'); ?>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <?php echo htmlspecialchars($registro['unidade'] ?? '-'); ?></td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <?php echo htmlspecialchars($registro['consumo_m3'] ?? '-'); ?></td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <?php echo htmlspecialchars($registro['multa'] ?? '-'); ?></td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <?php echo htmlspecialchars($registro['valor_fatura'] ?? '-'); ?></td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <?php echo htmlspecialchars($registro['total'] ?? '-'); ?></td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <?php echo htmlspecialchars($registro['secretaria'] ?? '-'); ?></td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <?php echo htmlspecialchars($registro['tipo_consumo'] ?? '-'); ?></td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <?php echo htmlspecialchars($registro['num_instalacao'] ?? '-'); ?></td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <?php echo htmlspecialchars($registro['data_vencimento'] ?? '-'); ?></td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <?php echo htmlspecialchars($registro['observacao'] ?? '-'); ?></td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <?php if ($registro['status'] === 'pago'): ?>
                                    <span
                                        class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800">Pago</span>
                                <?php else: ?>
                                    <span
